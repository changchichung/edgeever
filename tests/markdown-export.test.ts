...
describe("Markdown export", () => {
  ...
  test("preserves notebook hierarchy and disambiguates sibling names", () => {
    const paths = buildNotebookExportPaths([
      ...
      notebook({ id: "nb_child", name: "項目", parentId: "nb_root" }),
      notebook({ id: "nb_child_2", name: "項目", parentId: "nb_root" }),
    ]);

    expect(paths.get("nb_child")).toBe("工作/項目");
    expect(paths.get("nb_child_2")).toBe("工作/項目 (2)");
  });
  ...
  test("writes front matter, Markdown, and relative assets into the ZIP", async () => {
    ...
    const markdownPath = "工作/項目/項目筆記.md";
    const assetPath = "工作/項目/項目筆記.assets/設計 圖.png";
    ...
    expect(markdown).toStartWith(buildMarkdownFrontMatter(notes[0], "工作/項目"));
