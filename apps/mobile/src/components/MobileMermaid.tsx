...
export const MobileMermaidDiagram = ({
  ...
  if (!normalizedSource) {
    return <Text style={[styles.message, theme === "dark" && styles.messageDark]}>{locale === "en-US" ? "This Mermaid diagram is empty." : "此 Mermaid 圖表暫無內容。"}</Text>;
  }

  const hasResult = context?.results.has(normalizedSource) ?? false;
  const svg = context?.results.get(normalizedSource) ?? null;
  if (!hasResult) {
    return <Text style={[styles.message, theme === "dark" && styles.messageDark]}>{locale === "en-US" ? "Rendering diagram…" : "正在渲染圖表…"}</Text>;
  }
  if (!svg) {
    return <Text accessibilityRole="alert" style={styles.error}>{locale === "en-US" ? "Unable to render this Mermaid diagram. Check its syntax." : "無法渲染此 Mermaid 圖表，請檢查語法。"}</Text>;
  }
  ...
  return (
    <View accessibilityLabel={locale === "en-US" ? "Mermaid diagram" : "Mermaid 圖表"} accessible style={[styles.diagram, theme === "dark" && styles.diagramDark, { height }]}>
